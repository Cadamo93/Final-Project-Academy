...
namespace Z75.ClientPodatoci
{
    public partial class Manager : Form
    {
        ...
        public Manager()
        {
            ...
            listView3.Columns.Add("UserName", 100);
            listView3.Columns.Add("Date",100);
            listView3.Columns.Add("VremeRabota", 100);
            listView3.Columns.Add("VremePauza", 100);
            listView3.Columns.Add("VremeRucek", 100);
        }
        ...
        private async void btnShowTime_Click(object sender, EventArgs e)
        {
            listView1.Hide();
            listView2.Hide();
            listView3.Show();
            ...
            {
                // Connect to the server
                ConnectToServer(8070);

                // Send a request to the server to retrieve data from the SQLite table
                SendDataToServer("GetTimeSheet");

                listView3.Invoke((MethodInvoker)(() => listView3.Items.Clear()));
                ...
                if (!string.IsNullOrEmpty(responseData))
                {
                    ...
                    foreach (string row in rows)
                    {
                        ...
                        if (dataItems.Length >= 3)
                        {
                            ...
                            // Add the ListViewItem to listView1
                            listView3.Invoke((MethodInvoker)(() => listView3.Items.Add(item))); // Invoke UI update
                        }
                        else
                        {
                            MessageBox.Show("All data received.");
